...
- 响应式布局 
  - 当屏幕宽度大于900px的时候，即大型电脑PC端 设置默认CSS
  - 写当屏幕宽度在小于等于600px时，即在移动端，其对应的头部、尾部部分的CSS代码：
    @media screen and (max-width:600px){css settings}
- 自适应布局
  <meta name="viewport" content="width=devidce-width",user-scalable=no,initial-scale=1.0,maximum-scale=1.0,minimun-scale=1.0">
- 通过指定百分比宽度来替代 ，不使用像素大小，使用相对大小的字体 
- 流动布局 ，使用float，float的好处是，如果宽度太小，放不下两个元素，后面的元素会自动滚动到前面元素的下方，不会在水平方向overflow（溢出），避免了水平滚动条的出现。 
